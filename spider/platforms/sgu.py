...
"""
    ...
    def problem_parse(self, response, pid, url):
        ...
        match_groups = soup.find(name='div', attrs={'class': 'ttypography'})
        problem.html = ''
...
class SGU(Base):
    ...
    def get_problem(self, pid, account=None):
        if len(str(pid)) < 2 or str(pid).isdigit() is False:
            problem = Problem()
            problem.remote_oj = SGU.__name__
            problem.remote_id = pid
            problem.status = Problem.Status.STATUS_ERROR
            return problem
        p_url = f'https://codeforces.com/problemsets/acmsguru/problem/99999/{str(pid)}'
        res = self._req.get(url=p_url)
        return SGUParser().problem_parse(res, pid, p_url)
    ...
    def submit_code(self, account, pid, language, code):
        ...
        res = self._req.get('https://codeforces.com/problemsets/acmsguru/submit')
        if res is None:
            return Result(Result.Status.STATUS_SPIDER_ERROR)
        soup = BeautifulSoup(res.text, 'lxml')
        csrf_token = soup.find(attrs={'name': 'X-Csrf-Token'}).get('content')
        ...
        res = self._req.post(f'https://codeforces.com/problemsets/acmsguru/submit?csrf_token={csrf_token}',
                             data=post_data)
    ...
    def get_result(self, account, pid):
        if self.login_website(account) is False:
            return Result(Result.Status.STATUS_RESULT_ERROR)
        request_url = 'https://codeforces.com/problemsets/acmsguru/status?friends=on'
                ...
                for tr in list_tr:
                    if isinstance(tr, element.Tag) and tr.get('data-submission-id'):
                        return self.get_result_by_url(
                            f'https://codeforces.com/problemsets/'
                            f'acmsguru/submission/99999/{tr.get("data-submission-id")}')
        return Result(Result.Status.STATUS_RESULT_ERROR)

    # 根据源OJ的运行id获取结构
    def get_result_by_rid_and_pid(self, rid, pid):
        return self.get_result_by_url(f'https://codeforces.com/problemsets/'
                                      f'acmsguru/submission/99999/{rid}')

    # 根据源OJ的url获取结果
    def get_result_by_url(self, url):
        res = self._req.get(url=url)
        return SGUParser().result_parse(response=res)
...
if __name__ == '__main__':
    sgu = SGU(proxies='socks5://127.0.0.1:1086')
    print(sgu.get_problem(547).__dict__)
    # for key, value in sgu.get_problem(547).__dict__:
    #     print(key, value)
